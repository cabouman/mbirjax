...
The ``MultiAxisParallelBeamModel`` class implements a geometry and reconstruction model for parallel beam but with the ability to rotate in azimuth and tilt in elevation.
...
of functions that can be implemented using the ``MultiAxisParallelBeamModel``.
...
.. automethod:: mbirjax.MultiAxisParallelBeamModel.fbp_recon
