"""
tests/geometries/test_banded_projectors.py
────────────────────────────────────────────
Unit tests for the banded BACK projector, parametrized across ALL projection
geometries (cone, multiaxis-parallel, translation): the per-band kernel
``<Model>.back_project_one_view_to_band`` and the production back projector
``<Model>.back_project_one_view_to_pixel_batch`` that is built on it.

The production back projector hoists the horizontal fan (computed once per view) and walks
the recon slice axis in UNIFORM bands with a rolled jax.lax.map (+ reshape/crop).  This
test checks that it reproduces an INDEPENDENT banded assembly -- an explicit
np.concatenate of ``back_project_one_view_to_band`` over a NON-uniform tiling (which
recomputes the horizontal fan per band) -- across several band sizes (including one that
does not divide the slice count, exercising the padded-last-band crop) and coeff_power 1
(projection) and 2 (Hessian).  Agreement validates the horizontal-hoist, the lax.map
reassembly, and the banded vertical fan's tiling-invariance.

The per-band / production kernels (back_project_one_view_to_band,
back_project_one_view_to_pixel_batch, compute_channel_coordinate) are class methods with
identical signatures in every geometry, so the shared helpers and test bodies here are
geometry-agnostic: each config dict stores its ``model_class`` and the kernels are called
on that stored class.  Each geometry keeps its own ``make_config`` factory reproducing its
exact physical setup, and both of its variants:

  * CONE (ConeBeamModel): a CIRCULAR and a HELICAL geometry, so the anchor's
    helical-z-shift term is covered (a helical view has a nonzero per-view z shift).
  * MULTIAXIS (MultiAxisParallelModel): an ISOTROPIC and an ANISOTROPIC
    (voxel_row_aspect / voxel_slice_aspect != 1) geometry, with nonzero elevations, so the
    per-view tilt and the anisotropic slice pitch are both covered.
  * TRANSLATION (TranslationModel): an ISOTROPIC and an ANISOTROPIC
    (voxel_slice_aspect != 1) geometry with nonzero z translations, so the anchor's
    per-view z-shift term (translation_vector[2]) and the slice-aspect term are covered.

The banded vertical fan's own physical correctness (anchor, footprint, per-view z shift /
elevation tilt) is gated INDEPENDENTLY per geometry by the adjoint identity in
test_projectors and (for cone/translation) the convergence gate in test_vcd, both of which
run the production banded back projector.

Computed floats -> tight allclose (project rule: never exact equality for computed floats).
Band reassembly only reorders summation, so CPU agreement is ~1e-6; the gates use 1e-4 to
stay robust on GPU (projector scatter-add noise ~1e-6..8e-6 rel).
"""
import unittest
...
from mbirjax import ConeBeamModel, TranslationModel
...
# ── per-geometry config factories ────────────────────────────────────────────────
def make_cone_config(num_views, num_det_rows, num_det_channels, helical):
    ...
    return {
        'name': 'cone/helical' if helical else 'cone/circular',
...
def make_multiaxis_config(num_views, num_det_rows, num_det_channels, anisotropic):
    ...
    return {
        'name': 'multiaxis/anisotropic' if anisotropic else 'multiaxis/isotropic',
        'model_class': mj.MultiAxisParallelModel,
        'pp': make_projector_params(model), 'svp': svp, 'idx': idx, 'model': model,
        # The kernels take this view's CONCRETE integer channel centers as input.
...
def make_translation_config(num_views, num_det_rows, num_det_channels, anisotropic):
    ...
    return {
        'name': 'translation/anisotropic' if anisotropic else 'translation/isotropic',
...
class TestBandedProjector(unittest.TestCase):
    """Banded back-projector gates parametrized over every geometry + variant.  Each
    config stores its ``model_class``; the shared helpers/tests call the class-method
    kernels on it, so the bodies are geometry-agnostic."""
    ...
    def setUpClass(cls):
        cls.configs = [
            make_cone_config(9, 24, 32, helical=False),
            make_cone_config(9, 24, 32, helical=True),
            make_multiaxis_config(9, 24, 32, anisotropic=False),
            make_multiaxis_config(9, 24, 32, anisotropic=True),
            make_translation_config(9, 24, 32, anisotropic=False),
            make_translation_config(9, 24, 32, anisotropic=True),
        ]
    ...
    def _back_banded_concat(self, c, view, coeff_power=1):
        ...
        bands = [np.asarray(model_class.back_project_one_view_to_band(
            ...
            with self.subTest(geometry=c['name']):
                ...
                for coeff_power in (1, 2):
                    ...
                    for bs in band_sizes:
                        prod = np.asarray(model_class.back_project_one_view_to_pixel_batch(
