...
def compute_sino_and_params(dataset_dir, downsample_factor=(1, 1), subsample_view_factor=1, crop_pixels_sides=0, crop_pixels_top=0, crop_pixels_bottom=0, alu_unit=None, bg_option="global", zinger_correction=True, verbose=1):
    """
    Compute sinogram and parameters from txrm file generated by a Zeiss Ultra or Versa scanner.
    The Ultra scanner is treated as parallel-beam, and the Versa scanner is treated as cone-beam.

    The function returns``zeiss_metadata['scanner_type']`` as one of the following 3 values:
    'ultra': Xradia Ultra data; 'versa': Xradia Versa data; 'unknown': The system type cannot be determined.
    ...
    Args:
        dataset_dir (str): Path to the Zeiss ``.txrm`` dataset, which contains
            ``ImageData*/Image*`` (scan data) and Zeiss OLE metadata streams.
        downsample_factor (tuple[int, int], optional): Downsample factors for detector rows
            and channels. Defaults to ``(1, 1)``.
        subsample_view_factor (int, optional): Factor by which to subsample views. Defaults to ``1``.
        crop_pixels_sides (int, optional): Pixels to crop from each lateral side of the detector.
            Defaults to ``0``.
        crop_pixels_top (int, optional): Pixels to crop from the top of the detector. Defaults to ``0``.
        crop_pixels_bottom (int, optional): Pixels to crop from the bottom of the detector.
            Defaults to ``0``.
        alu_unit (str, optional): The physical unit used to define 1 ALU (Arbitrary Length Unit).
            Defaults to ``None``, in which case it is set to ``'um'`` for Ultra scans and ``'mm'``
            for Versa scans. Supported values: ``'um'``, ``'mm'``, ``'cm'``, ``'m'``.
    ...
    Returns:
        tuple: A tuple ``(sino, geometry_params, optional_params, zeiss_metadata)`` where

        - ``sino`` (numpy.ndarray): Sinogram of shape ``(num_views, num_det_rows, num_channels)``.
        - ``geometry_params`` (dict): Parameters for initializing the tomography model.
        - ``optional_params`` (dict): Additional parameters to be set via ``TomographyModel.set_params``.
        - ``zeiss_metadata`` (dict): Metadata stored in the Zeiss txrm file.
    ...
    return sino, geometry_params, optional_params, zeiss_metadata
...
def convert_zeiss_to_mbirjax_params(zeiss_params, downsample_factor=(1, 1), crop_pixels_sides=0, crop_pixels_top=0, crop_pixels_bottom=0, alu_unit=None):
    """
    ...
    Args:
        ...
        alu_unit (str, optional): The physical unit used to define 1 ALU (Arbitrary Length Unit). Defaults to None.
            If None, set to 'um' for Ultra scans and 'mm' for Versa scans.
    ...
    # Determine ALU unit based on scanner type
    if alu_unit is None:
