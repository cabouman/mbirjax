...
def segment_plastic_metal(recon, num_metal, sharpness=1.0, edge_threshold=0.003, radial_margin=10, top_margin=10, bottom_margin=10):
    ...
    for i in range(num_classes - 1):
        # Compute Gaussian-like log likelihoods between adjacent classes
        ...
        weights = weights.at[i].set(jnp.where(soft_mask, wi, weights[i]))
        weights = weights.at[i + 1].set(jnp.where(soft_mask, wj, weights[i + 1]))
