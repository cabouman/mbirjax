...
class FanBeamModel(TomographyModel):
    ...
    def __init__(self, sinogram_shape, angles, source_detector_dist, source_iso_dist, helical_z_shifts=None):
        ...
        if helical_z_shifts is None:
            ...
            try:
                ...
                raise ValueError(
                    "Incompatible view dependent vector lengths:  all view-dependent vectors must have the "
                    "same length.")
    ...
    def get_psf_radius(self):
        ...
        if jnp.isinf(source_detector_dist):
            ...
            source_to_closest_pixel = source_to_iso_dist - 0.5 * jnp.maximum(recon_shape[0],
                                                                             recon_shape[1]) * delta_voxel
            max_magnification = source_detector_dist / source_to_closest_pixel
            source_to_farthest_pixel = source_to_iso_dist + 0.5 * jnp.maximum(recon_shape[0],
                                                                              recon_shape[1]) * delta_voxel
    ...
    def forward_vertical_fan_one_pixel_to_one_view(voxel_cylinder, pixel_index, single_view_params, projector_params):
        ...
        def create_det_column_rows(start_index):
            ...
            for k_offset in jnp.arange(start=-gp.bp_psf_radius, stop=gp.bp_psf_radius + 1):
                ...
                abs_delta_p_r_m = jnp.abs(
                    m_p + slope_k_to_m * k_offset - m_center)  # Distance from projection of center of voxel to center of detector
                A_row_k = jnp.clip((W_p_r + 1) / 2 - abs_delta_p_r_m, 0,
                                   L_max)  # Fraction of the detector hit by this voxel
    ...
    @staticmethod
    def back_vertical_fan_one_view_to_one_pixel(detector_column_values, pixel_index, single_view_params,
                                                projector_params,
                                                coeff_power=1):
        """
    ...
    def split_sino_recon(self, sino, weights=None, half_overlap=5, init_recon=None, max_iterations=15,
