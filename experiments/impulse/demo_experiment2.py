...
def run_experiment(num_views, num_det_rows, num_det_channels, impulse_position):
    geometry_type = 'parallel'  # Choose between 'parallel' or 'cone'
...
# **Visualization**
def plot_sinogram_sums(angles, sinogram_sums, param_name, param_value):
    ...
    plt.title(f"Sum of Sinogram Entries vs View Angle ({param_name})")
    plt.grid(True)
    plt.legend()
    plt.show()

# **Experiment 1: Vary num_views**
for num_views in [32, 64, 128]:
    angles, sinogram_sums = run_experiment(num_views, 40, 128, (32, 32, 30))
    plot_sinogram_sums(angles, sinogram_sums, 'num_views', num_views)

# **Experiment 2: Vary num_det_channels**
for num_det_channels in [64, 128, 256]:
    angles, sinogram_sums = run_experiment(64, 40, num_det_channels, (32, 32, 30))
    plot_sinogram_sums(angles, sinogram_sums, 'num_det_channels', num_det_channels)

# **Experiment 3: Vary num_det_rows**
for num_det_rows in [20, 40, 80]:
    angles, sinogram_sums = run_experiment(64, num_det_rows, 128, (32, 32, 30))
    plot_sinogram_sums(angles, sinogram_sums, 'num_det_rows', num_det_rows)

# **Experiment 4: Vary impulse_position**
for impulse_position in [(32, 32, 30), (10, 10, 10), (50, 50, 40)]:
    angles, sinogram_sums = run_experiment(64, 40, 128, impulse_position)
    plot_sinogram_sums(angles, sinogram_sums, 'impulse_position', impulse_position)
