...
def main():
    ...
    W = np.random.rand(detector_rows * detector_columns, num_materials)  # Random initialization of W
    H = np.random.rand(num_materials, gt_hyper_projection.shape[2])  # Random initialization of H
    T = np.exp(-noisy_hyper_projection).reshape(-1, gt_hyper_projection.shape[2])
    learning_rate = 0.2
    ...
    denoised_hyper_projection = (W @ H).reshape(detector_rows, detector_columns, gt_hyper_projection.shape[2])
