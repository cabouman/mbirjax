"""
back_project sharding: correctness and performance across sizes and device counts.

Verifies that back_project produces bit-compatible results when the sinogram
is pre-sharded across devices, and reports timing for each configuration.
...
# results[(size, n_devices)] = {'time': float, 'max_diff': float, 'gpu_mem': dict}
results = {}
...
for size in SIZES:
    ...
    _ = model.back_project(sinogram_np)
    baseline = np.asarray(model.back_project(sinogram_np))

    times_base = []
    for _ in range(N_TIMING_RUNS):
        t0 = time.time()
        out = model.back_project(sinogram_np)
    ...
    for n_devices in N_DEVICES_LIST:
        ...
        # Shard the sinogram along axis 1 (det_rows / slices) before passing to
        # back_project.  This mirrors the usage in fbp_recon where fbp_filter
        # returns an already-sharded filtered_sinogram.
        sinogram_sharded = model._maybe_shard(sinogram_np, axis=1)

        # Warm up: triggers JIT compilation for this device count and sharding.
        _ = model.back_project(sinogram_sharded)

        # Correctness: compare sharded output to unsharded baseline.
        # Use np.asarray() to materialize the sharded output before comparing;
        # comparing JAX arrays directly can silently mask the JAX GPU scatter bug
        # (see .claude/status.md for details).
        recon_sharded = np.asarray(model.back_project(sinogram_sharded))
        max_diff = float(np.max(np.abs(recon_sharded - baseline)))

        # Timing.
        times = []
        for _ in range(N_TIMING_RUNS):
            t0 = time.time()
            out = model.back_project(sinogram_sharded)
            jax.block_until_ready(out)
...
ax.set_title('back_project sharding speedup')
ax.set_xticks(all_n)
