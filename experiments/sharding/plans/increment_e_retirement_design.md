...
> **STATUS: E1 + E2 + E4 + E3a + E3b DONE (E3a/E3b 2026-06-22, CPU-validated); E3f (folded into E5) + E5 pending.**  Written after a read-only
...
| **E3c — ⏳ TODO** — shard `denoising.py` | The QGGMRF denoiser commits its image-domain arrays (`flat_error_image`, `flat_image`) to a single device (`sino_placement.devices[0]`) and runs the update loop single-device.  Shard it like `vcd_recon` (slice-shard the image-domain arrays via the placement path; run the loop sharded), so the denoiser scales with the recon.  Gate: full suite + a sharded-vs-single-device denoise identity. | `denoising.py` | full suite green; sharded == single-device denoise |
| **E3f — ⏳ DEFERRED (folded into E5)** — the remaining vacuous `if is_sharded` collapse | The cosmetic dead-code sweep: the `if not self.is_sharded: return x` early-outs (`_shard_on_axis`, `_gather_to_host`, `_extract_halos`, `_stage_halos`), the `self.is_sharded and X` → `X` guard simplifications (padding/halo helpers, `_device_report`/`direct_filter`/error-sinogram/`fm_hessian` branches), and the `_device_report` `' (sharded)'` suffix (always present now).  None reference `main_device`, so they don't block E3b; they read as "is_sharded might be False" to a future reader and should go.  Pure dead-code removal (is_sharded ≡ True) — separate gate. | `tomography_model.py` (~15 `if is_sharded` sites) | full suite green; no behavior change |
| **E5 — the RETIRE-marker sweep + E3f + doc close-out** | `grep -rn "RETIRE"` and clear each: retire the trivial-mesh-vs-legacy comparison tests (`test_{back,forward}_projection.py`, `test_fbp_recon.py`, `test_vcd_sharded.py` — they compared the placement path to a legacy path that no longer exists; keep/relax to the sharded-vs-single-shard identities that remain meaningful); update the comments that referenced the retiring machinery; **do E3f** (the vacuous `if is_sharded` collapse); **decide the `configure_sharding` hard-delete** (rename ~75 test callers → `configure_devices` + drop the alias, vs. keep the alias); update `v3` §5 item 4 + `sharding_status.md`. | the 7 files with RETIRE markers; the plan docs | full suite green; no stale RETIRE markers |
