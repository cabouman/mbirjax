...
def main():
    ...
    if os.path.exists(out_file):
        ans = input('Overwrite existing {}?'.format(out_file))
        if ans != 'y':
            print('Exiting.')
            return
    ...
    n_cpus = 1  # np.round(n_cpus / 2).astype(int) + 1
    ...
    # Print key model parameters so they can be compared against other branches.
    print(f"  recon_shape               = {recon_shape}")
    print(f"  use_ror_mask              = {_ref_model.use_ror_mask}")
    print(f"  num_pixels                = {_all_indices_np.shape[0]}")
    print(f"  pixel_batch_size_for_vmap = {_ref_model.pixel_batch_size_for_vmap}")
    print(f"  view_batch_size_for_vmap  = {_ref_model.view_batch_size_for_vmap}")
    print(f"  flat_recon[:3, 0]         = {_flat_recon_np[:3, 0]}")
    print()
    ...
    for op_name, bench_fn, input_key, shard_axis in OPERATIONS:
        ...
        for n_dev in device_counts:
            ...
            if isinstance(raw_np, tuple):
                data_np, aux_np = raw_np
    ...
    # Include the projection inputs and key model scalars so that any branch
    # can verify it is feeding exactly the same data to its projector.
    baseline_refs['flat_recon']  = _flat_recon_np
    baseline_refs['all_indices'] = _all_indices_np
    baseline_refs['recon_shape'] = np.array(recon_shape)
    baseline_refs['use_ror_mask']              = np.array(_ref_model.use_ror_mask)
    baseline_refs['pixel_batch_size_for_vmap'] = np.array(_ref_model.pixel_batch_size_for_vmap)
    baseline_refs['view_batch_size_for_vmap']  = np.array(_ref_model.view_batch_size_for_vmap)

    np.savez(out_file, **baseline_refs)
