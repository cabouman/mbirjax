...
XLA_FLAGS="--xla_dump_to=/tmp/hlo --xla_dump_hlo_as_text" \
  python plans/experiments/sharding/vmap_lax_map_demo.py
```
...
- `plans/experiments/sharding/vmap_lax_map_demo.py` — annotated demo,
  reproduces the bug with `max|diff| ≈ 1.68` and shows the fix.
- `plans/experiments/sharding/minimal_lax_map_repro.py` — minimization
  script (this document's companion).  Contains a self-contained
  `gen_pixel_partition` so it has no mbirjax dependency.
- `plans/experiments/sharding/laxmap_isolation_test.py` — earlier in-tree
