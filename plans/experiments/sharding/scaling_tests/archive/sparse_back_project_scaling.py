"""
plans/experiments/sharding/scaling_tests/sparse_back_project_scaling.py
─────────────────────────────────────────────────────────────────
...
Run from the BETA worktree root:
    ...
    python plans/experiments/sharding/scaling_tests/sparse_back_project_scaling.py
"""
import os
import sys
import argparse
...
def worker_measure(size_label, device_counts, warmup, trials, out_file, path="band",
    ...
    def build_and_time(n, devs):
        ...
        # measure the reduce-scatter + compute, not the entry scatter.
        sino = model._shard_sinogram(sino_np)
        idx = make_indices(model)
        stats, _ = sc.time_op(lambda: run_back_project(model, sino, idx), warmup, trials)
    ...
    sc.run_measure_loop(size_label, device_counts, out_file, build_and_time,
                        header_extra=f" | path={path} | B_p={bp_label}")
