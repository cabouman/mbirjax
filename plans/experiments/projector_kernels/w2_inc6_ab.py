...
TRIALS = 3
VCD_ITERS = 4
OUT_DIR = '/scratch/gautschi/buzzard/w2_inc6_ab'
CELLS = []
for n in (1, 2, 4):
    for cfg in ('off', 'on'):
        CELLS.append(dict(name=f'cone_fwd_n{n}_{cfg}', n=n, op='fwd', on=cfg == 'on'))
for cfg in ('off', 'on'):
    CELLS.append(dict(name=f'cone_vcd_n2_{cfg}', n=2, op='vcd', on=cfg == 'on'))

if os.environ.get('W2I6_SMOKE') == '1':
    SINO_SHAPE = (32, 16, 24)
    VCD_ITERS = 1
    OUT_DIR = os.path.join(os.environ.get('TMPDIR', '/tmp'), 'w2_inc6_smoke')
    CELLS = [c for c in CELLS if c['n'] <= int(os.environ.get('W2I6_SMOKE_MAX_N', '1'))]
...
def worker(cfg):
    ...
    if cfg['op'] == 'fwd':
        # The user-facing full forward (recon cylinders in, sinogram out).
        idx = mbirjax.gen_full_indices(recon_shape, use_ror_mask=True)
        vals = jnp.asarray(np.random.default_rng(1).random(
            (len(idx), recon_shape[2]), dtype=np.float32))
        for _ in range(WARMUP):
            jax.block_until_ready(model.sparse_forward_project(vals, idx))
        ts = []
        for _ in range(TRIALS):
            t = time.perf_counter()
            r = jax.block_until_ready(model.sparse_forward_project(vals, idx))
            ts.append(time.perf_counter() - t)
        # Floor-calibrated single-shot contract (E6 design): max-rel 3e-4.
        result, tol = np.asarray(r), 3e-4
    else:
        ...
        # INFO-only: the max-norm VCD comparison is intrinsically noise-amplifying
        # (findings: "inc5 VCD divergence"); equivalence is gated by the occasional
        # convergence gate.  Walls are the deliverable here.
        result, tol = np.asarray(out), float('inf')
...
if __name__ == '__main__':
    worker(json.loads(os.environ['W2I6_CELL'])) if os.environ.get('W2I6_CELL') \
        else orchestrator()
